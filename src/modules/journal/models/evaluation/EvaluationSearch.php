<?php

namespace app\modules\journal\models\evaluation;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\modules\journal\models\evaluation\Evaluation;

/**
 * EvaluationSearch represents the model behind the search form about `app\modules\journal\models\evaluation\Evaluation`.
 */
class EvaluationSearch extends Evaluation
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['id', 'system_id', 'created_at', 'updated_at'], 'integer'],
            [['value'], 'safe'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Evaluation::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'system_id' => $this->system_id,
            'created_at' => $this->created_at,
            'updated_at' => $this->updated_at,
        ]);

        $query->andFilterWhere(['like', 'value', $this->value]);

        return $dataProvider;
    }
}
